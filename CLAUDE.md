...
- `ui/template/` — Page layouts: `MainLayout`, `PageLayout`(Image/Minimal/Brand/Plain — `navItems`를 넘기면 본문 최상단에 `SdBreadcrumb`가 **기본으로** 붙는다(모든 변형은 공유 `Content`를 거치고, `Content`가 브레드크럼을 그린다). `breadcrumb={false}`로 끄고, `currentHref`로 경로를 강제(생략 시 `usePathname()` 추론)한다. `navItems`가 없으면 아무것도 안 그린다)

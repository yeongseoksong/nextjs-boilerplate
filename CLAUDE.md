...
- `ui/molecule/` — Composite components: `SdTextBox`, `SdFeatures`, `SdSteps`, `SdTestimonial`, `SdPricingCard`, `SdFaq`, `SdCta`, `SdSolution`, `SdSolutionCard`, `SdClients`, `SdMap`, `SdBreadcrumb`(네비 계층 브레드크럼 — `navItems`의 `parentId` 트리에서 현재 경로 위치를 찾아 `홈 아이콘 > 조상 > … > 현재` 렌더. `currentHref` 생략 시 `usePathname()` 자동 추론, 정확 일치 없으면 최장 접두 매칭. 변형 없는 단일 컴포넌트라 flat export가 곧 `SdBreadcrumb` 자체 — 서버 컴포넌트에서 바로 렌더 가능. 프레임워크에서 유일하게 `next/navigation`을 import하므로 `tsup.config.ts` ui 외부 목록에 `next/navigation`이 명시돼 있다)
...
- `ui/template/` — Page layouts & providers: `MainLayout`, `PageLayout`(Image/Minimal/Brand/Plain — `navItems`를 넘기면 본문 최상단에 `SdBreadcrumb`가 **기본으로** 붙는다(모든 변형은 공유 `Content`를 거치고, `Content`가 브레드크럼을 그린다). `breadcrumb={false}`로 끄고, `currentHref`로 경로를 강제(생략 시 `usePathname()` 추론)한다. `navItems`가 없으면 아무것도 안 그린다), `SdProvider`(프로바이더/컨텍스트 조립만 — `MantineProvider`(테마) + `SdToastProvider` + `NavProvider`. 헤더/푸터 크롬인 `MainLayout`은 담지 않는다 — 소비자가 `SdProvider` 안쪽에 배치한다. `companyInfo`/`loginFlag`/`headerVariant`는 `MainLayout` prop이고, `SdProvider`는 `navItems`/`theme`/`defaultColorScheme`만 받는다. 회사명/로고는 여전히 env var, `ColorSchemeScript`/CSS는 `<head>` 몫이라 여기 없다), `NavProvider`/`useNav`(navItems를 Context에 얹고 읽는 한 쌍 — zustand가 아니라 Context인 이유는 §State Management 참고)
...
| `useNavStore`  | `store/nav.store.ts`     | `navItems` · `setNavItems` — 렌더 트리 **밖**에서 네비게이션 상태 접근용 (라우팅 로직 등) |
...
이게 안전한 이유는 **`ui/index.tsx`가 스토어를 재수출하지 않기 때문**이다. tsup(esbuild)은 상대 경로 import를 만나면 그 모듈을 인라인 복사한다 — 만약 `ui`가 `export * from '../store'`를 했다면 스토어 코드가 `dist/ui`와 `dist/store` 두 번들에 각각 있게 되고 `create()`도 두 번 호출되어, 소비자가 `/store`에서 가져온 스토어와 어떤 `ui` 컴포넌트가 내부에서 쓰는 스토어가 서로 다른 인스턴스가 된다 — `setCompanyName()`이 조용히 no-op이던 것과 같은 dual-bundle 모듈 상태 분열이다. 그래서 **지금 `ui/` 안의 어떤 컴포넌트도 스토어를 직접 import하지 않는다** — 이 불변식이 깨지면(예: `SdHeader`가 나중에 `useAuthStore`를 내부에서 읽도록 바뀌면) 스토어도 `ui`의 자체 entry로 다시 인라인해야 한다. `form.state.ts → ui/atom/Toast`처럼 반대 방향(store가 ui를 가져오는 것)은 문제없다 — Toast는 상태 없는 얇은 래퍼라 두 번들에 복사돼도 안전하다. 검증은 `dist/ui/index.mjs`에 `zustand`/스토어 코드가 0번, `dist/store/index.mjs`에 스토어별로 `create()`가 정확히 한 번씩 나오는지로 한다. 카탈로그(`apps/web/app/state/StateCatalog.tsx`)도 이 네 훅을 `@framework/ui`가 아니라 `@framework/store`에서 가져온다 — `Showcase`의 `from` prop이 코드 예시의 import 경로를 결정한다(기본값 `/ui`).

**트리 안 컴포넌트가 네비게이션을 읽는 경로는 zustand가 아니라 React Context다.** `ui/template/NavProvider.tsx`가 `NavProvider`(navItems를 Context에 얹음)와 `useNav()`(그 값을 읽음)를 제공하고, `SdBreadcrumb`는 `navItems` prop이 없으면 `useNav()`로 폴백한다. 이게 zustand가 아닌 이유가 위 불변식의 핵심이다 — `SdBreadcrumb`는 `ui` 번들 안에 있어서, 만약 `useNavStore`를 직접 import하면 `dist/ui`에 스토어가 인라인돼 `create()`가 두 번 도는 바로 그 dual-bundle 분열이 난다. React Context는 모듈 상태가 없어 `dist/ui`에 복사돼도 안전하므로 이 트리-안 경로에 맞는 도구다. 그래서 `useNavStore`(zustand)는 트리 **밖** 접근 전용으로 남고, 트리 **안** 배선은 Context가 맡는 — 두 저장소가 공존한다. `SdProvider`가 `NavProvider`로 자식을 감싸므로 그 아래 `SdBreadcrumb`는 prop 없이 동작한다.
